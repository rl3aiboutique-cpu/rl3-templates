...
**Date:** 2026-05-03 07:06:47 UTC
**Sandbox:** `/tmp/rl3-validation`
**Template ref:** `42d7046` on `feature/lehidalgo/v0.2.1-doc-refresh`
...
| Test | Status | Detail |
|---|---|---|
| CLAUDE.md exists | ✓ PASS | 88 lines |
| .pre-commit-config.yaml exists | ✓ PASS | 360 lines |
| .claude/settings.json exists | ✓ PASS | 97 lines |
| .gitleaks.toml exists | ✓ PASS | 35 lines |
| .yamllint.yaml exists | ✓ PASS | 35 lines |
| .codespellrc exists | ✓ PASS | 9 lines |
| .copier-answers.yml exists | ✓ PASS | 24 lines |
...
| Test | Status | Detail |
|---|---|---|
| branch-naming hook absent | ✓ PASS | correctly omitted |
| no-direct-push hook absent | ✓ PASS | correctly omitted |
| branch-source-base hook absent | ✓ PASS | correctly omitted |
| no-commit-to-branch scoped to production only | ✓ PASS | main-only |
| claude deny-list trimmed correctly | ✓ PASS | no git+develop, --no-verify and --force kept |
| CLAUDE.md uses LIGHTWEIGHT branching section | ✓ PASS | lightweight branding |
