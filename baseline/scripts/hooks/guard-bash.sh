...
# recoverable destructive ops (reset --hard, clean -f, checkout .) where
# blocking creates more friction than the protection is worth.
warn() {
...
# Forcing the agent to commit a lockfile before any install adds more
# friction than it saves; the lockfile shows up in the next commit anyway
# and is reviewable in PR.
